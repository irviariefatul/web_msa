    ...
    <div class="form-row">
        @foreach ($ivts as $index2 => $ivt)
            <div class="form-group col-md-7">
                <label for="InvestmentName-{{ $index2 }}">Investment Name<span class="text-danger">*</span></label>
                <select wire:model="ivts.{{ $index2 }}.id" class="form-control select2"
                    name="ivts[{{ $index2 }}][id]" required id="InvestmentName-{{ $index2 }}">
                    <option value="">Select Options</option>
                    @foreach ($investments as $i)
                        <option value="{{ $i->id }}">
                            {{ $i->nama_invest }} || Rp{{ number_format($i->harga, 0, ',', '.') }}
                        </option>
                    @endforeach
                </select>
            </div>
            <div class="form-group col-md-2">
                ...
                <input wire:model="ivts.{{ $index2 }}.estimasi_ivts" type="number" class="form-control"
                    ...
                    placeholder="Estimation" required step="1">
            </div>
            <div class="form-group col-md-2">
                ...
                <div class="input-group">
                    <input wire:model="ivts.{{ $index2 }}.pemeliharaan_ivts" type="number" class="form-control"
                        ...
                        placeholder="Maintenance Cost (%)" step="0">
                    <div class="col-md-2">
